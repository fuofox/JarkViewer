...
`Debug` 用于本地调试，`Release` 对应发布构建。第三方静态库不会自动还原；构建前请先解压 `static_lib` 发布包到约定位置，或在项目属性中启用 `vcpkg`，并按 `README.md` 中列出的包手动安装依赖。

## 代码风格与命名规范
遵循 `JarkViewer/src` 现有 C++ 风格：4 空格缩进，大括号与语句同行，源码文件使用 UTF-8。`#include` 按逻辑分组，优先复用现有模式，不随意新增抽象层。类型名使用 `PascalCase`，函数和局部变量使用 `camelCase`，宏和既有常量使用 `ALL_CAPS`。

## 测试要求
当前仓库没有自动化测试套件。每次改动至少要保证 `Debug|x64` 可干净编译。若涉及行为变更，请在程序内进行对应功能的手动冒烟验证，例如静态图加载、动图播放、EXIF 解析、打印预览和导出流程。
